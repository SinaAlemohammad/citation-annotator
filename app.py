...
# OTP via Resend is no longer used (kept here harmlessly in case re-enabled later).
...
def login_page():
    ...
    st.markdown("# Welcome.")
    st.markdown(
        "Enter your email to begin. You'll stay signed in for 30 days on this browser."
    )
    ...
    looks_like_email = "@" in email and "." in email.split("@")[-1] if email else False

    if st.button("Continue", type="primary", disabled=not looks_like_email):
        log_in(email)
        st.rerun()
